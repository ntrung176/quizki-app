...
import { Calendar, Clock, Target, ChevronLeft, RotateCcw, RotateCw, BarChart3, Cpu, Play } from 'lucide-react'
...
const KanjiReviewScreen = ({ awardXP, setIsReviewActive, isAdmin = false }) => {
    ...
    const stats = useMemo(() => {
        ...
        return {
            dueToday: dueKanji.length, newCards: hasNoSRS, learning, shortTerm, longTerm, expert,
    ...
    const handleRating = (rating) => {
        ...
        // 2. Perform Firestore writes asynchronously in the background with retry
        ...
        (async () => {
            ...
            while (attempts < 3 && !success) {
                try {
                    await setDoc(doc(db, `artifacts/${appId}/users/${userId}/kanjiSRS`, currentCard.id), newSrs, { merge: true });
                    ...
                    attempts++;
                    console.error(`Error updating Kanji SRS in background (attempt ${attempts}):`, e);
                    if (attempts < 3) {
                    ...
                    <div className="w-full pt-4">
                        {newlyDueKanji.length > 0 ? (
                            <div className="flex flex-col sm:flex-row items-center gap-3 w-full">
                                ...
                                >
                                    <RotateCw className="w-4 h-4" />
                                    <span>Tiếp tục ôn tập ({newlyDueKanji.length} thẻ)</span>
                                </button>
                                <button
                                    onClick={(e) => { e.stopPropagation(); exitReview(true); }}
