...
import { X, Volume2, CheckCircle, ChevronLeft, ChevronRight, PenTool, BookOpen, Lightbulb, Info } from 'lucide-react';
import { speakKorean } from '../../utils/audio';

const HangulCardModal = ({
    item,
    isOpen,
    onClose,
    onNext,
    ...
    const [activeTab, setActiveTab] = useState('detail'); // 'detail' | 'mnemonic' | 'examples'

    if (!isOpen || !item) return null;

    const handlePlayAudio = (textToSpeak) => {
        speakKorean(textToSpeak || item.char);
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-950/75 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto animate-fade-in">
            <div
                ...
                className="relative w-full max-w-lg bg-white dark:bg-slate-900 border-2 border-cyan-400/30 dark:border-cyan-500/20 rounded-3xl shadow-2xl overflow-hidden flex flex-col my-auto transition-all"
            >
                {/* Header with gradient banner */}
                <div className="relative bg-gradient-to-r from-cyan-600 via-sky-600 to-blue-600 p-5 text-white flex items-center justify-between">
                    <div className="flex items-center gap-3.5">
                        <div className="min-w-[56px] h-14 px-2 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-white shadow-inner font-sans shrink-0">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <span className="text-lg font-mono font-black tracking-wide">/{item.romaji}/</span>
                            ...
                            <p className="text-xs text-sky-100 font-medium mt-0.5">
                                {item.pronunciationGuide || 'Cách phát âm chuẩn tiếng Hàn'}
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex items-center border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/60 p-1.5 gap-1">
                    ...
                    >
                        ...
                        <span>Nét vẽ ({item.strokes || 1} nét)</span>
                    </button>
                    <button
                        type="button"
                ...
                <div className="p-5 overflow-y-auto max-h-[60vh] space-y-4">
                    {/* TAB 1: STROKE ORDER & ARTICULATION */}
                    {activeTab === 'detail' && (
                        <div className="space-y-4 animate-fade-in">
                            {/* Big Character Display Card */}
                            <div className="flex flex-col items-center justify-center p-6 rounded-3xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 relative">
                                <span className="text-7xl font-black font-sans text-slate-900 dark:text-white select-none">
                                    {item.char}
                                </span>
                            ...
                            {item.strokeHints && item.strokeHints.length > 0 && (
                                <div className="w-full bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800 space-y-2">
                                    <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                                        Quy tắc thứ tự nét viết chuẩn:
                                    </span>
                                    <ul className="space-y-1.5 text-xs text-slate-700 dark:text-slate-300 font-medium">
                                        {item.strokeHints.map((hint, idx) => (
                                            <li key={idx} className="flex items-start gap-2">
                                                <span className="w-4 h-4 rounded-full bg-cyan-100 dark:bg-cyan-950/80 text-cyan-600 dark:text-cyan-400 font-bold flex items-center justify-center text-[10px] shrink-0 mt-0.5">
                                                    {idx + 1}
                                                </span>
                                                <span>{hint}</span>
                                            </li>
                            ...
                            <div className="p-4 rounded-2xl bg-sky-500/10 border border-sky-500/20 flex items-start gap-3">
                                ...
                                <div className="space-y-0.5">
                                    <h5 className="text-xs font-bold text-sky-800 dark:text-sky-300">Hướng dẫn phát âm:</h5>
                                    <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                                        {item.pronunciationGuide || 'Luyện tập phát âm tròn vành rõ chữ cùng nút loa phía trên.'}
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 animate-fade-in">
                            <div className="p-5 rounded-2xl bg-amber-500/10 border border-amber-500/25 flex items-start gap-3.5">
                                ...
                                <div className="space-y-1">
                                    <h4 className="text-sm font-bold text-amber-700 dark:text-amber-300">
                                        Mẹo hình tượng hóa chữ {item.char} ({item.romaji})
                                    </h4>
                                    <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed font-medium">
                                        {item.mnemonic || `Hình dung chữ ${item.char} để dễ dàng nhận diện và ghi nhớ sâu trong trí não.`}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 text-center space-y-2">
                                <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                                    Ký tự & Phiên âm Quốc tế:
                                </span>
                                <div className="flex items-center justify-center gap-6 text-3xl font-black font-sans py-2">
                                    <span className="text-cyan-600 dark:text-cyan-400">{item.char}</span>
                                    <span className="text-slate-300 dark:text-slate-700 text-xl">↔</span>
                                    <span className="text-indigo-600 dark:text-indigo-400 font-mono text-2xl">/{item.romaji}/</span>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'examples' && (
                        <div className="space-y-3 animate-fade-in">
                            <span className="text-xs font-bold text-slate-500 block px-1">
                                Các từ vựng Topik thông dụng chứa chữ cái này:
                            </span>
                            {item.examples && item.examples.length > 0 ? (
                                item.examples.map((ex, idx) => (
                                    <div
                                        ...
                                        className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/80 flex items-center justify-between hover:border-cyan-400 hover:shadow-md transition-all cursor-pointer group"
                                    >
...
export default HangulCardModal;
