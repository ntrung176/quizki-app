...
const AppContent = () => {
    ...
    return (
        <div className={`min-h-screen font-sans ${isDarkMode ? 'dark text-slate-100' : 'text-slate-900'} relative transition-colors duration-200`}>
            ...
            {/* Main view container */}
            <main className={`min-h-screen flex flex-col transition-[padding,margin] duration-200 ease-out ${userId ? (isSidebarCollapsed ? 'pt-[calc(3.75rem+env(safe-area-inset-top,0px))] lg:pt-0 lg:pl-20' : 'pt-[calc(3.75rem+env(safe-area-inset-top,0px))] lg:pt-0 lg:pl-64') : 'pl-0'}`}>
                ...
                <div className={`${isReviewSessionPage ? 'w-full flex-1 flex flex-col items-center justify-center bg-transparent py-4 md:py-8' : ['KANJI', 'KANJI_STUDY', 'KANJI_REVIEW', 'KANJI_SAVED', 'VOCAB_REVIEW', 'VOCAB_LIST', 'VOCAB_ADD', 'VOCAB_QUICK_ADD', 'BOOKS', 'JLPT_TEST', 'JLPT_ADMIN', 'VIDEO_KAIWA'].includes(view) || location.pathname.startsWith('/vocab/set') || location.pathname.startsWith('/vocab/edit-set') || location.pathname.startsWith('/jlpt') || location.pathname.startsWith('/grammar') || location.pathname.startsWith('/kaiwa') ? 'w-full flex-1 flex flex-col p-0' : 'w-full max-w-6xl mx-auto px-3 md:px-4 py-4 md:py-6 flex-1 flex flex-col'}`}>
