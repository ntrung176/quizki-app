    ...
    return (
        <div className={`min-h-screen font-sans ${isDarkMode ? 'dark text-slate-100' : 'text-slate-900'} relative transition-colors duration-200`}>
            ...
            <main className={`min-h-screen flex flex-col w-full max-w-full min-w-0 overflow-x-hidden transition-[padding,margin] duration-200 ease-out ${userId ? (isSidebarCollapsed ? 'pt-[calc(3.5rem+env(safe-area-inset-top,0px))] lg:pt-0 lg:pl-20' : 'pt-[calc(3.5rem+env(safe-area-inset-top,0px))] lg:pt-0 lg:pl-64') : 'pl-0'}`}>
                ...
                <div className={`${isReviewSessionPage ? 'w-full flex-1 flex flex-col items-center justify-center bg-transparent py-4 md:py-8' : ['KANJI', 'KANJI_STUDY', 'KANJI_REVIEW', 'KANJI_SAVED', 'VOCAB_REVIEW', 'VOCAB_LIST', 'VOCAB_ADD', 'VOCAB_QUICK_ADD', 'BOOKS', 'JLPT_TEST', 'JLPT_ADMIN', 'VIDEO_KAIWA', 'KANA', 'HANGUL', 'IPA'].includes(view) || location.pathname.startsWith('/books') || location.pathname.startsWith('/vocab') || location.pathname.startsWith('/kanji') || location.pathname.startsWith('/jlpt') || location.pathname.startsWith('/grammar') || location.pathname.startsWith('/kaiwa') || location.pathname.startsWith('/kana') || location.pathname.startsWith('/hangul') || location.pathname.startsWith('/ipa') ? 'w-full max-w-full min-w-0 flex-1 flex flex-col p-0' : 'w-full max-w-6xl mx-auto px-3 md:px-4 py-4 md:py-6 flex-1 flex flex-col'}`}>
                    <div className={`w-full max-w-full min-w-0 flex-1 flex flex-col ${isReviewSessionPage || ['KANJI', 'KANJI_STUDY', 'KANJI_REVIEW', 'KANJI_SAVED', 'VOCAB_REVIEW', 'VOCAB_LIST', 'VOCAB_ADD', 'VOCAB_QUICK_ADD', 'BOOKS', 'JLPT_TEST', 'JLPT_ADMIN', 'VIDEO_KAIWA', 'KANA', 'HANGUL', 'IPA'].includes(view) || location.pathname.startsWith('/books') || location.pathname.startsWith('/vocab') || location.pathname.startsWith('/kanji') || location.pathname.startsWith('/jlpt') || location.pathname.startsWith('/grammar') || location.pathname.startsWith('/kaiwa') || location.pathname.startsWith('/kana') || location.pathname.startsWith('/hangul') || location.pathname.startsWith('/ipa') ? 'bg-transparent' : ''}`}>
                        <div className={`w-full max-w-full min-w-0 flex-1 flex flex-col ${isReviewSessionPage || ['KANJI', 'KANJI_STUDY', 'KANJI_REVIEW', 'KANJI_SAVED', 'VOCAB_REVIEW', 'VOCAB_LIST', 'VOCAB_ADD', 'VOCAB_QUICK_ADD', 'BOOKS', 'JLPT_TEST', 'JLPT_ADMIN', 'VIDEO_KAIWA', 'KANA', 'HANGUL', 'IPA'].includes(view) || location.pathname.startsWith('/books') || location.pathname.startsWith('/vocab') || location.pathname.startsWith('/kanji') || location.pathname.startsWith('/jlpt') || location.pathname.startsWith('/grammar') || location.pathname.startsWith('/kaiwa') || location.pathname.startsWith('/kana') || location.pathname.startsWith('/hangul') || location.pathname.startsWith('/ipa') ? 'bg-transparent' : ''}`}>
                            <AppRoutes
                                authReady={authReady}
